...
```{r linewidth=80}
source_url("https://raw.githubusercontent.com/katphelps/36-350/main/README.md")
```
...
source_url("https://raw.githubusercontent.com/katphelps/36-350/main/dark_and_stormy.R")
```
...
```{r linewidth=80}
#called it branch_q3 for clarities sake
source_url("https://raw.githubusercontent.com/katphelps/36-350/main/dark_and_stormy.R")
source_url("https://raw.githubusercontent.com/katphelps/36-350/branch_q3/dark_and_stormy.R")
```
...
```{r linewidth=80}
source_url("https://raw.githubusercontent.com/katphelps/36-350-Fork/main/check_mark.R")
```
---
...
test_that(desc = "Test that output is double",                  expect_type(f("a"),"double"))
test_that(desc = "Test that lower-case letters work",            expect_equal(f("a"), 0))
test_that(desc = "Test that upper-case letters work",            expect_equal(round(f("b"), 3), 0.693))
#the below are meant to fail
test_that(desc = "Test that only one letter is allowed",            expect_error(f("ab")))
test_that(desc = "Test that it only works on letters",expect_error(f(0)))
```
...
f = function(letter) {
  ...
  if ( nchar(letter) > 1 ) stop("The input character must be of length 1.")
  return(log(which(letters==letter)))
}
...
#the below are meant to fail
test_that(desc = "Test that only one letter is allowed",            expect_error(f("ab")))
test_that(desc = "Test that it onyl works on letters",expect_error(f(0)))
```